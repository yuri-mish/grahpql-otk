  ...
  fields: {
    ...
    buyers_orders: {
      ...
      resolve: async function (par, args, cont, info) {
       ...
       var queryOptions={currUser:cont.currUser}
       console.log('queryOptions:',queryOptions)  
       var qq = createQuery(args, info, "buyers_order", BuyersOrderType,50,queryOptions);
    ...
    logout:{
      ...
      resolve: async (obj, args, context) => {
       // if (!context.currUser) return {}
        if (context.currUser.token){
          Users = _.remove(Users, function(n) { return n.token === context.currUser.token})
        } 
        return {}
    }  
    },  
      ...
      const POSTrequestOptions = {
       ...
       res = await fetch('https://couch.vioo.com.ua'+'/_session',POSTrequestOptions)
      .then(response => {
                 if(response.ok) {
             ...
             .then(data => {
                        ...
                        //context.res.cookie('token', data.token, {httpOnly:false,  'httpOnly': false,
                        //SameSite:false,})
                        if (!user){
                          ...
                          data.token = token
                          context.res.setHeader('Set-Cookie',[`token=${token};  Path=/; Max-age= 3600;SameSite=false;`]);  
                          Users.push({ref:ref,name:data.name,isAdmin:isAdmin,branch:branch,token:data.token,validTo:moment().add(3, 'hours')})
                        }
                        return data
             })
              .catch(error => console.log( '=Error=:'+error));
       if (res.ok)
         return {ok:res.ok, name:res.name, token: res.token}
